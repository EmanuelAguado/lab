...
def field_widget(field_name, type_):
    ...
    if type_ == int:
        ...
        widget = ComponentsWidget(PLUGINS)
        widget.value_changed = widget.currentTextChanged
        widget.value = widget.currentText
    elif type_ == GAddon:
        widget = ComponentsWidget(ADDONS)
        widget.value_changed = widget.currentTextChanged
        widget.value = widget.currentText
    elif type_ == GProject:
        widget = ComponentsWidget(PROJECTS)
        widget.value_changed = widget.currentTextChanged
        widget.value = widget.currentText
    elif type_ == GUser:
        widget = ComponentsWidget(USERS)
        widget.value_changed = widget.currentTextChanged
        widget.value = widget.currentText
    elif type_ == GStudio:
        widget = ComponentsWidget(STUDIOS)
        widget.value_changed = widget.currentTextChanged
        widget.value = widget.currentText
...
for find_class in [GPlugin, GAddon]:
    COMPONENTS[find_class] = dict()
    for name, class_ in dict_class.items():
        if issubclass(class_, find_class):
            COMPONENTS[find_class][name] = dict(
                class_.config_class.default().items()
            )
...
class FieldWidget(QTreeWidgetItem):
    def __init__(self, field, type_, parent, widget, *args, **kwargs) -> None:
        ...
        if parent:
            if isinstance(parent, GDictConfigWidget):
...
class GListConfigWidget(QListWidget):
...
class GDictConfigWidget(QTreeWidget):
    on_resize = Signal(int)
    def __init__(self, field, cfg, data, parent, *args, **kwargs):
        super().__init__(*args, **kwargs)
        ...
        if isinstance(data, list):
            self.config_widgets[field] = GListConfigWidget(field, cfg, data, self)
        elif isinstance(data, dict):
            for sub_field, sub_type in self._data.items():
                type_, args_ = detect_type(sub_type)
                if type_ == list:
                    self.config_widgets[sub_field] = GListConfigWidget(
                        sub_field, args_, sub_type, self
                    )
                elif type_ == dict:
                    self.config_widgets[sub_field] = GDictConfigWidget(
                        sub_field, args_, sub_type, self
                    )
                else:
    ...
    def setItemWidget(self, item, column, widget):
        ...
        if isinstance(widget, GListConfigWidget):
            widget.on_resize.connect(
    ...
    def add_component(self):
        ...
        print("add component:",data,type_,class_)
        component = [
            comp
            for comp in ADDONS.list_components
            + PLUGINS.list_components
            + PROJECTS.list_components
            + USERS.list_components
            + STUDIOS.list_components
            if id_ == comp.id
        ]
        if component:
            component[0].name = name
        else:
            if issubclass(class_, GAddon):
                ADDONS.add_component(Component(id_, name, type_))
            elif issubclass(class_, GPlugin):
                PLUGINS.add_component(Component(id_, name, type_))
            elif issubclass(class_, GProject):
                PROJECTS.add_component(Component(id_, name, type_))
            elif issubclass(class_, GUser):
                USERS.add_component(Component(id_, name, type_))
            elif issubclass(class_, GStudio):
                STUDIOS.add_component(Component(id_, name, type_))
...
class GConfigWindow(QMainWindow):
    ...
    def create_widgets(self):
        ...
        load_button = QPushButton("Load", clicked=self.load_config)
        apply_button = QPushButton("Apply", clicked=self.apply_changes)
    ...
    def create_config(self, name: str, cfg: GComponent, type_: type):
        ...
        if type_ == dict:
            tree = GDictConfigWidget(name, cfg, data, None)
        else:
            tree = GListConfigWidget(name, cfg, data, None)
...
if __name__ == "__main__":
    ...
    window = GConfigWindow(config_data)
    window.show()
    app.exec()
